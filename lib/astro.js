...
async function syncdb() {
  ...
  let AstaConn = AstaConnectSock({
    ...
    browser: ["Windows", "chrome", "MAXTECH_MD-V2"],
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "MAXTECH_MD-V2") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "MAXTECH_MD-V2") => {
...
app.listen(randomPort, () =>
  console.log(
    "MAXTECH_MD-V2 Server listening on http://localhost:" + randomPort + "/ "
  )
);
